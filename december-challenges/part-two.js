function partTwo(str) {
  ...
  const firstDigitMatch = str.match(
    /\d|(?:one|two|three|four|five|six|seven|eight|nine)/
  );
  const lastdigitsMatch = str.match(
    /.*(\d|(?:one|two|three|four|five|six|seven|eight|nine))/
  );
  let v1 = "";
  let v2 = "";
  if (isNaN(+firstDigitMatch[0])) {
    v1 += digitsObj[firstDigitMatch[0]];
  } else {
    v1 += firstDigitMatch[0];
  }

  if (isNaN(+lastdigitsMatch[1])) {
    v2 += digitsObj[lastdigitsMatch[1]];
  } else {
    v2 += lastdigitsMatch[1];
  }

  return `${v1}${v2}`;
}
function finalTest(inputPuzzle) {
  const lines = inputPuzzle.split("\n");
  const arrNumber = [];
  for (let i = 0; i < lines.length; i++) {
    arrNumber.push(partTwo(lines[i]));
  }
  return arrNumber.reduce((acc, cur) => Number(acc) + Number(cur), 0);
}
const strings = `gtlbhbjgkrb5sixfivefivetwosix
